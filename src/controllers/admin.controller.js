/**
 * Admin Controller — адміністратор
 * CRUD ігор, введення результатів
 */
const scheduleService = require('../services/schedule.service');

/* Dashboard — список усіх ігор */
...
async function saveResult(req, res) {
    try {
        await scheduleService.saveResult(req.params.id, req.body.team1Score, req.body.team2Score);
        res.redirect('/admin');
    } catch (err) {
        console.error(err);
        res.status(500).send('Помилка сервера');
...
module.exports = { dashboard, createForm, createGame, editForm, updateGame, deleteGame, resultForm, saveResult };
